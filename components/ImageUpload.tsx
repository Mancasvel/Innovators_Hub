...
export default function ImageUpload({
  ...
  const { startUpload, isUploading: isUploadingUT } = useUploadThing(uploadType, {
  ...
  const uploadImages = useCallback(async () => {
    ...
    try {
      ...
      const filesToUpload = previewImages.map(previewImage => previewImage.file);

      console.log('🚀 Starting upload of', filesToUpload.length, 'files to', uploadType);
      console.log('📁 Files:', filesToUpload.map(f => ({ name: f.name, size: f.size, type: f.type })));
      ...
      if (!uploadedFiles) {
        throw new Error('UploadThing returned null or undefined. Check server logs for authentication errors.');
      }

      if (uploadedFiles.length === 0) {
        throw new Error('No files were uploaded - UploadThing returned empty array. Check file size and format restrictions.');
      }
      ...
      if (multiple) {
        ...
        console.log('💾 Updating images array:', { old: currentImages, new: newImages });
        onChange(newImages);
      } else {
      ...
      if (err instanceof Error && err.message.includes('No secret provided')) {
        setError('UploadThing token not configured. Please check your .env.local file.');
      } else {
  ...
  }, [previewImages, isUploading, isUploadingUT, multiple, normalizedValue, onChange, uploadType, startUpload]);
  ...
  return (
    <div className="space-y-6">
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex gap-2">
              ...
              >
                {(isUploading || isUploadingUT) ? 'Uploading...' : `Upload ${previewImages.length} Image${previewImages.length > 1 ? 's' : ''}`}
              </button>
            </div>
          </div>
      ...
      {(!multiple && !normalizedValue && previewImages.length === 0) ||
       (multiple && (normalizedValue as string[]).length + previewImages.length < maxImages) && (
